window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ShieldIcon = ({ size = 28 }) => h("svg", {
      width: size,
      height: size,
      viewBox: "0 0 24 24",
      fill: "none",
      stroke: "var(--dsw-alias-brand, #3b82f6)",
      strokeWidth: "2",
      strokeLinecap: "round",
        ...
        h("div", { style: dshStyles.modal },
          h("div", { style: dshStyles.brandHeader },
            h("div", { style: dshStyles.logoWrap },
              FishLogo ? h(FishLogo, { size: 28 }) : h(ShieldIcon, { size: 28 })
            ),
    ...
    function AuthGuardSettings() {
      ...
      return h("div", { style: dshStyles.section },
        h("div", { style: { display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" } },
          FishLogo ? h(FishLogo, { size: 22 }) : h(ShieldIcon, { size: 22 }),
    ...
    exports.apply = function(ctx) {
      // 0. Auto-bridge LAN settings mirror and loopback context
      function syncLanLoopbackContext() {
        try {
          const conn = ctx.get("connection");
          if (conn && !conn.isLoopback) {
            Object.defineProperty(conn, "isLoopback", { value: true, configurable: true, writable: true });
          }
          const remote = ctx.get("remote");
          if (remote?.$host && !remote.$host.isLoopback) {
            Object.defineProperty(remote.$host, "isLoopback", { value: true, configurable: true, writable: true });
          }
          const binder = ctx.get("settingsScope");
          if (binder) {
            if (binder.persistence === "memory") binder.persistence = "host";
            if (binder.mirror?.persistence === "memory") {
              binder.mirror.persistence = "host";
              binder.mirror.load().catch(() => {});
            }
          }
        } catch (e) {}
      }

      syncLanLoopbackContext();
      if (typeof ctx.on === "function") {
        ctx.on("connection/reset", syncLanLoopbackContext);
      }
      if (typeof ctx.inject === "function") {
        ctx.inject(["connection"], syncLanLoopbackContext);
        ctx.inject(["remote"], syncLanLoopbackContext);
        ctx.inject(["settingsScope"], syncLanLoopbackContext);
      }
